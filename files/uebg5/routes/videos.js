...
videos.route('/')
    .get(function (req, res, next) {
        ...
        if (req.url == "/") {
            ...
            var selectionList = VideoModel.find({}, function(err, items) {
                res.json(items);
            });
            ...
            if (limitAndOffset.length > 13) {
                ...
                var selectionListWithMoreParams = VideoModel.find({}, function(err, items) {
                    res.json(items);
                });
                ...
                var selectionListWithOneParam = VideoModel.find({}, function(err, items) {
                    res.json(items);
                });
...
/**
 * Pause, hier weiter machen, Foliensatz 7 Seite 58/87
 */
videos.route('/:id')
    .get(function (req, res, next) {
